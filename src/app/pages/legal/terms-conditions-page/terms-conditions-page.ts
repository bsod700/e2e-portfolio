...
export class TermsConditionsPageComponent implements OnInit {
  ...
  private readonly cacheKey = 'legal_page_terms_conditions';
  ...
  private loadContent(): void {
    this.contentService
      .getLegalPageContent('terms-conditions')
      .pipe(take(1))
      .subscribe({
        next: (content) => {
          if (!content) {
  ...
  private tryLoadFromCache(): void {
    ...
    try {
      ...
      console.error('Error reading terms & conditions from cache:', error);
    }
  }
  ...
  private saveToCache(content: LegalPageContent): void {
    ...
    try {
      ...
      console.error('Error saving terms & conditions to cache:', error);
    }
  }
